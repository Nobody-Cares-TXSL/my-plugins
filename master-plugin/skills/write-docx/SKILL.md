...
| 表格 | [[references/tables.md]] | 三线表边框哲学、`columnWidths` 与 tblGrid 的关系、行列控制要点 |
...
| 遇到 bug | [[references/pitfalls.md]] | 32 条症状→原因→修复速查 |
...
- [ ] 精确列宽的表格同时设 `columnWidths`（只给 cell 设 `width` 会让 tblGrid 退化成 `gridCol w="100"`，中文在词中间折行）
